...
import ma.vi.esql.database.init.lookup.LookupInitializer;
import ma.vi.esql.extension.lookup.Lookup;
import ma.vi.esql.extension.lookup.LookupExtension;
 ...
 */
class LookupInitTest extends DataTest {
  @TestFactory
  Stream<DynamicTest> initLookups() {
    return Stream.of(databases)
                 .map(db -> dynamicTest(db.target().toString(), () -> {
                   ...
                   new LookupInitializer().add(db, "/init/test_lookups.yml", LookupInitTest.class.getResourceAsStream("/init/test_lookups.yml"));
