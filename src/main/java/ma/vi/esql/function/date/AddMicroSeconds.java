...
/**
 * Add microseconds to date.
 *
 * @author Vikash Madhow (vikash.madhow@gmail.com)
 */
public class AddMicroSeconds extends Function {
  public AddMicroSeconds() {
    super("addmicroseconds", Types.IntType,
  ...
  public String translate(FunctionCall call, Target target, EsqlConnection esqlCon, EsqlPath path, Environment env) {
    ...
    if (target == POSTGRESQL) {
      return args.get(0).translate(target)
           + "concat(" + args.get(1).translate(target) + ", ' microsecond')::interval";
    } else if (target == SQLSERVER) {
      return "dateadd(microsecond, "
