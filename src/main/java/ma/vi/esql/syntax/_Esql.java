 ...
 */
public class _Esql<V, R> implements Copy<_Esql<V, R>>, Translatable<R> {
  public _Esql(Context context,
               V value,
               T2<String, ? extends _Esql<?, ?>>... children) {
    this.value = value;
    this.context = context;
    if (value instanceof _Esql) {
      ((_Esql<?, ?>)value).parent = this;

    } else if (value instanceof List<?>) {
      for (Object e: (List<?>)value) {
        if (e instanceof _Esql) {
          ((_Esql<?, ?>)e).parent = this;
        }
      }
    }
    for (T2<String, ? extends _Esql<?, ?>> child: children) {
      child(child.a, child.b);
    }
  }

  public _Esql(Context context, V value, _Esql<?, ?>[] children) {
    this(context, value, fromArray(children));
  }

  public _Esql(Context context, V value, List<? extends _Esql<?, ?>> children) {
    this(context, value, children == null ? new _Esql<?, ?>[0] : children.toArray(new _Esql[0]));
  }

  public _Esql(_Esql<V, R> other) {
    context = other.context;

    // copy value
    if (other.value instanceof _Esql<?, ?>) {
      value = (V)((_Esql<?, ?>)other.value).copy();
      ((_Esql<?, ?>)value).parent = this;
    ...
    // copy children
    for (Map.Entry<String, ? extends _Esql<?, ?>> child: other.children.entrySet()) {
   ...
   */
  @Override
  public _Esql<V, R> copy() {
//    if (!copying()) {
//      try {
//        copying(true);
        return new _Esql<>(this);
//      } finally {
//        copying(false);
//      }
//    } else {
...
//    }
  ...
  private static T2<String, _Esql<?, ?>>[] fromArray(_Esql<?, ?>[] esqls) {
    T2<String, _Esql<?, ?>>[] array = new T2[esqls.length];
  ...
  public <T extends _Esql<?, ?>> T[] childrenArray(Class<T> childrenType) {
    int i = 0;
    T[] array = (T[])Array.newInstance(childrenType, children.size());
    for (_Esql<?, ?> e: children.values()) {
      array[i++] = (T)e;
    }
    return array;
  }

  public <T extends _Esql<?, ?>> List<T> childrenList() {
  ...
  public <T extends _Esql<?, ?>> _Esql<?, ?> childrenList(String childName, List<T> childrenList) {
    child(childName, new _Esql<>(context, childName, childrenList));
    return this;
  }

  public <T extends _Esql<?, ?>> T child(String child) {
    return (T)children.get(child);
  }

  public <T> T childValue(String child) {
    _Esql<?, ?> v = children.get(child);
    return v == null ? null : (T)v.value;
  }

  public <T> _Esql<T, ?> childValue(String child, T value) {
    _Esql<T, ?> v = (_Esql<T, ?>)children.get(child);
    if (v == null) {
      v = new _Esql<>(context, value);
      child(child, v);
    } else {
      v.value = value;
    }
  ...
  public <T extends _Esql<?, ?>> T child(String name, T child) {
   ...
   *                   This must be a subclass of {@link _Esql}.
   ...
   * or null if none.
   */
  public <T extends _Esql<?, ?>> T child(String name, T child, boolean hookParent) {
    if (child != null) {
      if (hookParent) {
        child.parent = this;
      }
   ...
   * or null if none.
   */
  public <T extends _Esql<?, ?>> T childCopy(String name, T child) {
  ...
  public _Esql<?, ?> remove(String name) {
...
//    return TranslatorFactory.get(target).translate(this, parameters);
   ...
   */
  public boolean forEach(Function<_Esql<?, ?>, Boolean> visitor,
                         Predicate<_Esql<?, ?>> explore) {
  ...
  private static boolean _forEach(_Esql<?, ?> esql,
                                  Function<_Esql<?, ?>, Boolean> visitor,
                                  Predicate<_Esql<?, ?>> explore,
                                  Set<_Esql<?, ?>> explored) {
    if (explore == null || explore.test(esql)) {
      explored.add(esql);
      if (esql.value instanceof _Esql && !explored.contains(esql.value)) {
        if (!_forEach((_Esql<?, ?>)esql.value, visitor, explore, explored)) {
          return false;
        }

      } else if (esql.value instanceof List) {
        for (Object e: (List<?>)esql.value) {
          if (e instanceof _Esql && !explored.contains(e)) {
            if (!_forEach((_Esql<?, ?>)e, visitor, explore, explored)) {
              return false;
            }
          }
        }
      }
      for (_Esql<?, ?> child: esql.children.values()) {
   ...
   */
  public _Esql<?, ?> map(Function<_Esql<?, ?>, _Esql<?, ?>> mapper,
                         Predicate<_Esql<?, ?>> explore) {
  ...
  private static _Esql<?, ?> _map(_Esql esql,
                                  Function<_Esql<?, ?>, _Esql<?, ?>> mapper,
                                  Predicate<_Esql<?, ?>> explore,
                                  Set<_Esql<?, ?>> explored) {
    if (explore == null || explore.test(esql)) {
      explored.add(esql);
      if (esql.value instanceof _Esql && !explored.contains(esql.value)) {
        esql.value = _map((_Esql)esql.value, mapper, explore, explored);

      } else if (esql.value instanceof List) {
        esql.value = ((List)esql.value).stream()
                                       .map(e -> e instanceof _Esql && !explored.contains(e)
                                                 ? _map((_Esql<?, ?>)e, mapper, explore, explored)
                                                 : e).collect(toList());
      }
      esql.children = ((Map<String, _Esql<?, ?>>)esql.children)
          .entrySet().stream()
  ...
  public _Esql<?, ?> mapChildren(Function<_Esql<?, ?>, _Esql<?, ?>> mapper) {
  ...
  private static _Esql<?, ?> _mapChildren(_Esql esql,
                                          Function<_Esql<?, ?>, _Esql<?, ?>> mapper,
                                          Predicate<_Esql<?, ?>> explore,
                                          Set<_Esql<?, ?>> explored) {
    if (explore == null || explore.test(esql)) {
  ...
  public _Esql<V, R> persistentMap(Function<_Esql<?, ?>, _Esql<?, ?>> mapper) {
    return persistentMap(mapper, x -> true);
  }
   ...
   * and all mutating operations creates a new persistent tree.
   *
   * @param mapper
   * @param explore
   * @return
  ...
  public _Esql<V, R> persistentMap(Function<_Esql<?, ?>, _Esql<?, ?>> mapper,
                                   Predicate<_Esql<?, ?>> explore) {
    if (explore.test(this)) {
      _Esql<V, R> mapped = (_Esql<V, R>)mapper.apply(this);
      if (mapped == null) {
        return null;
      }

      _Esql<V, R> copy = null;
      for (Map.Entry<String, _Esql<?, ?>> e: mapped.children.entrySet()) {
        String name = e.getKey();
        _Esql<?, ?> child = e.getValue();
        if (child != null) {
          _Esql<?, ?> mappedChild = child.persistentMap(mapper, explore);
          if (mappedChild != child) {
            if (copy == null) {
              copy = mapped == this ? mapped.copy() : mapped;
            }
            if (mappedChild == null) {
              copy.children.remove(name);
            } else {
              copy.children.put(name, mappedChild);
            }
          }
        }
      }
  ...
  public <T> T firstChild(Class<T> cls) {
    ...
    if (value instanceof _Esql<?, ?>) {
      T firstChild = ((_Esql<?, ?>)value).firstChild(cls);
      if (firstChild != null) {
        return firstChild;
      }
    }
    ...
    if (value instanceof List) {
      for (Object v: (List<?>)value) {
        if (v instanceof _Esql<?, ?>) {
          T firstChild = ((_Esql<?, ?>)v).firstChild(cls);
          if (firstChild != null) {
            return firstChild;
          }
        }
    ...
    for (_Esql<?, ?> child: children.values()) {
      if (child != null) {
   ...
   */
  public String replaceWith(_Esql<?, ?> replacement) {
    String thisChildName = null;
    if (parent != null) {
      if (parent.value == this) {
        parent.replaceWith(null, replacement);
      } else {
        for (Map.Entry<String, _Esql<?, ?>> child: parent.children.entrySet()) {
   ...
   */
  public Object replaceWith(String childName, _Esql<?, ?> replacement) {
  ...
  public void _toString(StringBuilder st, int level, int indent) {
    ...
    if (!children.isEmpty()) {
      st.append(" {\n");
      for (Map.Entry<String, _Esql<?, ?>> c: children.entrySet()) {
        _Esql<?, ?> v = c.getValue();
        if (v != null) {
...
//  }
   ...
   * have an empty map.
   */
  public Map<String, _Esql<?, ?>> children = new LinkedHashMap<>();
